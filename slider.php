...
<div id="jssor_1"
    style="position:relative;margin:0 auto;top:0px;left:0px;width:1600px;height:550px;overflow:hidden;visibility:visible;">
    <!-- Loading Screen -->
    ...
    <div data-u="slides"
        style="cursor:default;position:relative;top:0px;left:0px;width:1600px;height:550px;overflow:hidden;">
        <?php
        require 'connection.php';
        global $con;

        // Cache slider images for 30 minutes
        $slides = cache_remember('slider_images', function () use ($con) {
        ...
        if (!empty($slides)) {
            ...
            foreach ($slides as $res) {
                $slide_count++;
                $loading_attr = ($slide_count === 1) ? '' : ' loading="lazy"'; // First image loads immediately
                ?>
                <div data-p="680">
                    <?php
                    // Determine image path (check slider/ subdir first, then root images/)
                    $imgName = htmlspecialchars($res['img']);
                    $finalSrc = "images/slider/" . $imgName;
                    if (!file_exists($finalSrc)) {
                        if (file_exists("images/" . $imgName)) {
                            $finalSrc = "images/" . $imgName;
                        }
                    }
                    ?>
                    <img data-u="image" <?php echo $loading_attr; ?> src="<?php echo $finalSrc; ?>"
                        alt="<?php echo htmlspecialchars($res['title']); ?>" />

                    <?php if (!empty($res['title']) || !empty($res['subtitle'])) { ?>
                        <svg viewbox="0 0 750 100" data-to="50% 50%" width="750" data-t="7"
                            style="left:500px;top:600px;display:block;position:absolute;font-family:'Cormorant Garamond', serif;font-size:100px;font-weight:900;overflow:visible;">
    ...
    <!-- Bullet Navigator -->
    <div data-u="navigator" class="jssorb057" style="position:absolute;bottom:16px;right:16px;" data-autocenter="1"
